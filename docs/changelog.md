...
- `docs/peer-guide.md` 头部「当前协议版本 0.3.1」为主仓历史行文，未随 0.4.0 改名刷新（正文帧目录
  经核对与 0.4.0 schema 逐字段一致，帧形状零变化）。2026-09-16 已按 ADR-001 将该版本行改为指向
  `src/meta.ts`（消灭裸版本号），时序示例对齐 fixtures/v0.4 金样本、速查表补 0.4.0 行，其余正文
  保留平移原样。
- **发布线事实（2026-09-16 登记口径）**：npm 上的 `@kuro-bridge/protocol@0.1.0`（2026-09-15）系从
  **主仓 KuroAdapter** 的 `bridge/protocol` 发布（非本仓），包版本轴 0.1.0 与其内嵌协议版本 0.4.0
  错位，且 exports 缺 require。事故记录、主仓副本冻结与本仓 0.4.0 首发 / deprecate 计划见
  `docs/DECISIONS.md` ADR-001。本仓尚无任何发布。
- **发布线终态（2026-09-18）**：0.4.0 已从本仓发布（npm latest，2026-09-18T11:17Z）；0.1.0 已按
  ADR-001 措辞 deprecate。上条「尚无任何发布」为 09-16 登记口径，就此成为历史事实记载，不再修改。
